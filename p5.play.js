...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  * @return {Boolean} True if overlapping
  */
  this.collide = function(target, velocityCheck, callback) {
    return this._collideWith('collide', target, callback, velocityCheck);
  };
  ...
  */
   ...
   * @param {boolean} return will only be true if this sprite has a higher velocity than target when collision occurs
   * @return {boolean} true if a collision occurred
   */
  this._collideWith = function(type, target, callback, velocityCheck) {
    this.touching.left = false;
    this.touching.right = false;
    this.touching.top = false;
    this.touching.bottom = false;
...
function Group() {
  ...
  */
  ...
  function _groupCollide(type, target, callback) {
    ...
    for(var i = 0; i<this.size(); i++)
      didCollide = this.get(i)._collideWith(type, target, callback, velocityCheck) || didCollide;
    return didCollide;
  }
