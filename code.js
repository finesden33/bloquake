...

{
	...
	var skins = ["", "_ruby", "_emerald", "_gold", "_copper", "_charcoal", "_amythest", "_pink", 
				...
				"_crown", "_l", "_dittle", "_light", "_clock", "_champ", "_prime", "_orange", "_saw", "_cage", "_cry", "_potion", "_goldbox", "_skull"];
	var unlocked = [true]
	for (var i = 0; i < 56; i++) {
		unlocked.push(false)
	}
...
function achievementWatchers() {
	...
	if (!speedMode) {
		...
		if (stage >= 30 && hits == 0) achUnlock(53);

	} else { //SPEEDMODE ACHIEVEMENTS:
		//get to stage 20 in speed mode
		if (stage >= 20 && speedMode) achUnlock(43)
		//get to stage 40 in speed mode
		if (stage >= 40 && speedMode) achUnlock(44)
		//get to stage 60 in speed mode
		if (stage >= 60 && speedMode) achUnlock(45)
		//get to stage 80 in speed mode
		if (stage >= 80 && speedMode) achUnlock(46)
		//get to stage 100 in speed mode
		if (stage >= 80) achUnlock(47)
		//get a combo of 100 in speed mode
		if (topcombo >= 100 && speedMode) achUnlock(48)
		//get a combo of 200 in speed mode
		if (topcombo >= 200 && speedMode) achUnlock(49)
		//get a combo of 500 in speed mode
		if (topcombo >= 500 && speedMode) achUnlock(50)
		// reach super speed
		if (hspd >= 33.5 && speedMode && buff == "superKinetic") achUnlock(51)
		// game over with time left in speed mode
		if (hp <= 0 && score > 0 && speedMode) achUnlock(52)
	}
	...
	if (totalBroke + broke >= 10000) achUnlock(54);
	// destroy a total of 10,000 enemies
	if (badsTotal + thornsTotal + eyesTotal >= 10000) achUnlock(55)
...
} 
{
	...
	var achGoals = ["Default", "Use all of the power ups in one game", "Have 10 Thorn-Heads present at once", "Complete the tutorial", "Don't destroy the evil eye before stage 30", 
					...
					"Destroy 100 Thorn-Heads in total", "Destroy 1000 Bad Cubes in total", "Destroy 100 foes before stage 20", "Destroy 50 Evil Eyes in total", "Reach stage 15", "Reach stage 30", "Reach stage 45", "Reach stage 60", 
					...
					"Destroy an Evil Eye past stage 29", ":)", "???", "Reach stage 100", "Reach stage 20 in speed mode", "Reach stage 40 in speed mode", "Reach stage 60 in speed mode", "Reach stage 80 in speed mode", "Reach stage 100 in speed mode",
					"Get a 100 combo in speed mode", "Get a 200 combo in speed mode", "Get a 500 combo in speed mode", "Achieve ultra speed", "Game over in speed mode with time left", "Reach stage 30 without getting hit",
					"Break a total of 50,000 boxes", "Destroy a total of 10,000 enemies"];
	...
	var achNames = ["The starter costume", "You have the power within", "Overgrown", "You got schooled!", "Eye'll show you mercy", "Heart of coal", "Marathon slider", "That's oddly specific", "New beginnings", "One of the usuals", "Veteran player", 
					...
					"No flying boxes, thank you", "Seriously? That's silly", "Monster party!", "Levitating fireball shooting eyeball", "Never hide your smile", "A spicy surprise!", "The Bloquake Champion", 
					"Gotta go fast", "Need4speed", "Stayin' Alive at the speed of light", "Unbeatable clock beater", "The Bloquake Speed Legend", "Sequential Breakages", "Timer? What Timer?", "Combolord", "Flying across the screen", "Can't catch a break, eh?", "Invincible",
					"Serious dedication", "The Bloquake Menace"];
}
/*
Stage 
